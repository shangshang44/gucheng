		...
		$.ajax({
			...
			success:function(r){
				...
				$select.change(function(){
					var v = $(this).val();
					var t = $(this).find("option[value='" +v+"']")
					updateGtid(v, $select);
				})
			}
		})
...
//	alert("要修改的商品id=" + goodsid + "  类别id=" + gtid);
	//第1步：写一个web服务方法
	//第2步：在这里写ajax请求，调用上面的方法
	//第3步：在业务层接口扩展一个更新商品类别的方法
	//第4步：在业务层实现类实现相应的方法
	//第5步：在数据层DAO接口扩展一个更新商品类别的方法
	//第6步：在DAO对旧的mapper文件中定义相应的update语句
	//第7步：在客户端处理服务端返回的结果，显示消息气泡
...
function queding(a){
	...
	$.ajax({
		url : "admin/goods/update.php",  //最前不要斜杠
		type: "post",
		data: data,
		success : function(r){
